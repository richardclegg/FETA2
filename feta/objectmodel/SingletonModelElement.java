...
public class SingletonModelElement extends ObjectModelElement {
    
    
    public SingletonModelElement()
    {
    }
    ...
    public void calcNormalisationFrom(Network net, int [] from) {
        ...
        for (int i= 0; i < from.length; i++) {
            if (net.inLinks_.get(from[i]).length == 1)
                total--;
        }
        if (total == 0) {
            normalise_= 0.0;
    ...
    public double calcProbability(int nodeNo, Network net)
    {
        ...
        if (normalise_ == 0.0) {
            ...
            if (net.inLinks_.get(nodeNo).length == 1) {
